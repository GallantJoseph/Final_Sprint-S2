...
const Cart = () => {
	...
	const handleQuantityChange = async (id, quantityChange) => {
		...
		let updCartItem = cartDataContext.cartItems.filter((item) => item.id === id)[0];
		...
		if (updCartItem.quantity + quantityChange > 0) {
			...
			cartDataContext.setCartItems((prevCartData) =>
				prevCartData.map((item) => (item.id === id ? { ...item, id: id, quantity: item.quantity + quantityChange } : item))
			);
	...
	const handleRemoveItem = async (id) => {
		...
		cartDataContext.setCartItems((prevItems) => prevItems.filter((item) => item.id !== id));
	};
					...
					<div className="cartlist">
						{cartDataContext.cartItems.map((cartItem) => (
							<div key={cartItem.id} className="cartitem">
								<img src={productsData[cartItem.id - 1].image} alt={productsData[cartItem.id - 1].name} className="cartitemimage" />

								<div className="cartpartname">{productsData[cartItem.id - 1].name}</div>
								...
								<div className="cartprice">${productsData[cartItem.id - 1].price.toFixed(2)}</div>

								<button className="cartremove" onClick={() => handleRemoveItem(cartItem.id)}>
									X
								</button>
							</div>
						))}
							...
							<div className="cartlist">
								{pcBuildDataContext.buildItems.map((item, index) => (
									<div key={index} className="cartitem">
										<img src={productsData[item.id - 1].image} alt={productsData[item.id - 1].name} className="cartitemimage" />
										<div className="cartpartname">{productsData[item.id - 1].name}</div>

										<div className="cartcategoryname">{categoriesDict[productsData[item.id - 1].category]}</div>

										<div className="cartprice">${productsData[item.id - 1].price.toFixed(2)}</div>
									</div>
								))}
							</div>
						)}
